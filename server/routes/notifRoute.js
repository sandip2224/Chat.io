...
router.post('/register-push-device', async (req, res) => {
	...
	function isUnique() {
		return subModel.count({ where: { endpoint: subscription.endpoint, key_p256dh: subscription.keys.p256dh, key_auth: subscription.keys.auth, name: name, room: room } })
			.then(count => {
				if (count != 0) {
					return false;
				}
	...
	if (flag) {
		...
		const newSub = await subModel.create(subObj)
...
router.delete('/deregister-push-device', async (req, res) => {
	...
	await subModel.destroy({
		where: {
			endpoint: subscription.endpoint, key_p256dh: subscription.keys.p256dh, key_auth: subscription.keys.auth, name, room
		}
	})
...
router.post('/send-notification', async (req, res) => {
	...
	const subscriptions = await subModel.findAll({
		where: {
			room: msg.room
		}
	})
